...
//#include "color_jpg_test.h"
//#include "gray_jpg_test2.h"
...
extern volatile uint8_t jpeg_encode_done;//кодирование закончилось
...
static uint32_t  usbd_video_AltSet = 0;
...
static uint8_t  usbd_video_DataIn (void *pdev, uint8_t epnum)
{
  ...
  if (tx_enable_flag == 0)//если передача закончилась
  {
    if (jpeg_encode_done)//если кодирование закончилось
    {
      ...
      new_frame_cap_enabled = 1;//начать новое кодирование
