...
//0 - wait for sync low edge, 1 - wait for sync high edge
...
void ADC_IRQHandler(void)      
{
  ...
  if (sync_flag == 0)
  {
    ADC_AnalogWatchdogThresholdsConfig(ADC3, HIGH_ADC_THRESHOLD,0);//produce interrupt when > 42
    //начался спад
    //low edge begin
    curr_time = TIM2->CNT;
    ...
    if ((pulse_period > 62) && (pulse_period <66) && (capture_enabled == 2))
    {
      ...
      {
        //кадровые синхроимпульсы закончились, пошли пустые строки
        //frame sync ended
  ...
  {
    ADC_AnalogWatchdogThresholdsConfig(ADC3, 255,LOW_ADC_THRESHOLD);//produce interrupt when close to 0
    sync_flag = 0;
  }
...
//used for time counting between sync signals
//accuracy - 1us
void init_tim2(void)
{
